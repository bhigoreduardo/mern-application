...
export default function Profile({ user, isAdmin = false, _type }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-6 border border-gray-100 rounded-sm shadow-md pb-2 bg-white">
        <Heading
        ...
        />
        <FormProfile user={user} isAdmin={isAdmin} _type={_type} />
      </div>
      {Object.keys(user)?.length !== 0 && (
        <>
          <div className="flex flex-col gap-6 border border-gray-100 rounded-sm shadow-md pb-2 bg-white">
            <Heading title="Endereço" />
            <FormAddress user={user} isAdmin={isAdmin} _type={_type} />
          </div>
          {!isAdmin && (
            <div className="flex flex-col gap-6 border border-gray-100 rounded-sm shadow-md pb-2 bg-white">
